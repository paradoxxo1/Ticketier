...
const EditTicketForm = ({ticket}: Props) => {
  ...
  try {
    ...
    setLoading(true)
    await axios.put(`${process.env.NEXT_PUBLIC_BASE_URL}/edit/${ticket.id}`,data)
    router.push('/tickets');
    router.refresh()
    ...
    console.log(error)
    toast.error('Erorr on Editing new Ticket')
  }
  };
  ...
  },[setValue, ticket.from, ticket.passengerName, ticket.passengerSSN, ticket.price, ticket.time, ticket.to])
