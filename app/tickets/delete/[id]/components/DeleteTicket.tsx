...
const DeleteTicket = ({ ticket }: Props) => {
    ...
    const deleteHandlerFunction = async () => {
        try {
            setLoading(true);
            await axios.delete(`${process.env.NEXT_PUBLIC_BASE_URL}/delete/${ticket.id}`);
            router.push('/tickets');
            router.refresh();
            toast.success('Ticket Deleted successfully');
        } catch (error) {
            console.log(error);
            toast.error('Error on Deleting new Ticket');
        }
    };
    ...
    return (
        <>
            ...
            <div className={`border-2 dorder-red-200 w-96 max-w-full mx-auto mt-4 p-2 rounded-lg ${loading ? 'blur-xl' : ''}`} >
                <h1 className='mx-4 my-2 text-red-600 '>
                    Are You Sure Delete This Ticket Information{' '}
                </h1>
                <div className='m-4'>
